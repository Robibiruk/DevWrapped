...
export default function Deck({ story, username }: { story: Story; username: string }) {
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto bg-night text-slate-100" onClick={advance}>
      ...
      <div className="fixed left-4 top-4 z-20 flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
        <Link to="/" className="rounded-full border border-white/10 bg-white/5 p-2 text-slate-400 transition hover:text-white">
          <Home className="h-4 w-4" />
        </Link>
        <span className="font-mono text-xs text-slate-500">devwrapped.</span>
      </div>
      <div className="fixed right-4 top-4 z-20 font-mono text-xs text-slate-500" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="fixed bottom-5 left-1/2 z-20 flex -translate-x-1/2 gap-2" onClick={(e) => e.stopPropagation()}>
        {slides.map((_, i) => (
          <button
            key={i}
            onClick={() => goTo(i)}
